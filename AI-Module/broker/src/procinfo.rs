...
/// Send SIGTERM (graceful termination request), not SIGKILL -- gives the
/// target process a chance to exit cleanly (flush files, release
/// resources) rather than being killed outright. A well-behaved desktop
/// app closing in response to this is indistinguishable from the user
/// closing it themselves through its own UI.
pub fn terminate_process(pid: u32) -> Result<(), &'static str> {
    // Safety: `libc::kill` with a real, just-verified-to-exist pid and
    // SIGTERM is a standard, safe signal-delivery call; failure is
    // reported through the normal negative-return-value/errno convention,
    // handled below rather than assumed to succeed.
...
/// Per-interface network throughput, one snapshot entry per interface.
...
/// Per-interface throughput, sampled over `window` -- same two-read-with-a-
/// wait pattern as `cpu_usage_percent`. Loopback is excluded; interfaces
/// are reported with their kernel-reported up/down state
/// (`/sys/class/net/<iface>/operstate`) alongside throughput, since a
/// down interface with stale nonzero counters is a more useful "why is my
/// network slow" signal than throughput alone.
pub fn network_status(window: Duration) -> Vec<NetworkInterface> {
    ...
    for (name, (rx_after, tx_after)) in &after {
...
fn read_net_dev_totals() -> HashMap<String, (u64, u64)> {
